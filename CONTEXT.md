...
- **Audience broadening + readability pass:** removed CCIS-specific audience phrasing across user-facing copy so the product now targets students generally. Updated strings in `components/hero-section.tsx`, `components/features-section.tsx`, `components/footer.tsx`, `app/layout.tsx`, and `components/testimonials-section.tsx` (including testimonial reference IDs from `CCIS-*` to `STU-*` and heading text to “What students actually say”). Also improved low-contrast metadata/readability in verify surfaces by darkening faint labels in `components/verify/MessageBubble.tsx`, `components/verify/ResultCard.tsx`, and `components/verify/VerifyClient.tsx`, plus key hero micro-labels in `components/hero-section.tsx`. `pnpm lint` remains clean (`0`).
